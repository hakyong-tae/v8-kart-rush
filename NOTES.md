...
- **3가지 모드**:
  - **타임어택** (솔로 스피드전): 3랩 합계가 **코스별 글로벌 리더보드** 등록 (낮을수록 상위, 1인 1기록)
  - **멀티 스피드전**: 게이지 부스터 승부 (아이템 없음)
  - **멀티 아이템전**: 부스트/미사일/트랩 (기록 리더보드 미반영)
- **멀티플레이**: 룸 기반 최대 8인, 실시간 위치 동기화, 방장이 모드 선택
- **가드레일 벽**: 트랙 양쪽 벽 충돌 — 충격각 비례 감속+바운스, 강충돌 시 드리프트 캔슬
- **치비 라이더** 캐릭터 탑승, 뭉게구름, 부스터 시 스피드라인+FOV 펀치, 칩튠 BGM (WebAudio 시퀀서)
- 부스트 패드, 아이템 박스, 잔디 감속(숄더), 역주행 경고, R 리셋
