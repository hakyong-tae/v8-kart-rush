...
- **캐릭터 × 카트 분리 로스터**: 캐릭터 5종(모카 밸런스/코코 게이지/피코 안정/라임 스피드/토토 가속 — 외형 + 소폭 스탯 보너스 ±2~6%) × 카트 4종(스파크 R 밸런스 / 터보 G 가속 / 맥스 O 최고속 / 코멧 W 드리프트 — 스탯 ±3~18%) 자유 조합. **최종 스탯 = 캐릭터 × 카트 곱연산** (`combineStats`), 타이틀 화면에 회전하는 3D 조합 프리뷰 + 최종 스탯 바(100=기준) 실시간 표시. `roster.ts`에 추가만 하면 확장
- **아이템 2슬롯** (카트라이더식): 부스트 🚀 / 미사일 🎯 / 트랩 🟣 / **실드 🛡️**(10초간 1회 피격 방어, 버블 비주얼) / **번개 ⚡**(시전자 제외 전원 스핀, 실드로 방어 가능)
